...
// vr: renvoie par référence ! sinon tu copies
...
// vr: non: utilises true et false !
bool vrai=true;
bool faux=false;

// vr: il ne faut pas utiliser une liste pour y rechercher des éléments mais une unordered_map
// passe tes arguments par référence pour ne pas les recopier alors que tu veux juste les lire
bool is_in_lst(std::string elem, std::vector<std::string> lst){
  for (auto c:lst){
    if (c==elem) {
      return faux; // vr: si c==elem je pense que tu dois retourner true
    }
  }
  return vrai;  // vr: si c n'est pas sdans lst je pense que tu dois retourner false
}
...
// vr: juste pour compiler (à revoir)
Graphe_adj MAT_TO_ADJ(Graphe_mat graphe){ // vr: tentative pour compiler 
  std::vector<std::string> sommets=graphe.get_sommets(); //on récupère les sommets
  // vr: ne les recopie pas !
  std::vector<std::vector<double>> mat=graphe.get_mat(); //on récupère la matrice d'adjacence
...
// void Graphe_adj::print_graph() {  //afficher un graphe sous forme textuelle
...
void Graphe_adj::print_graph() { std::cout << "lets gooo" << std::endl; }

std::string Graphe_mat::print_gr_mat(){  //affiche la matrice d'adjacence
  // vr: graphe c'est l'objet sur lequel tu as appelé la méthode (son
  // adresse est this) donc l'objet contient l'attribut mat et sommets
  
  // attention en plus ici tu copies mat et sommet  
...
Graphe_adj read_graph_adj(){
   ...
   while(i<sz){
     // vr: attention ici ne redéfinis pas n ! il va cacher celui
     // défini juste avant: tu veux juste le remettre à 0
     // int n{0}; //nombre de "mots"
     std::string mot = "";
     // vr: attention un espace est ' ' " " est une chaîne de caractères donc un pointeur
     // donc non à while((graph[i]!=" ") && (graph[i]!='\n')){
...
// vr: tu ne dois pas parcourir la liste pour extraire l'index d'un élément: fait un dictionnaire (unordered_map)
// vr: ne copie pas les arguments ! passe les par référence
int ind_elem(std::string elem, std::vector<std::string> lst){ //fonction qui renvoie l'indice dans une liste lst d'un élément elem
  ...
  // vr: quel indice veux-tu retourner si le sommet n'est pas dans la liste ??
}
...
void parcourt_iteratif(Graphe_adj graph){
    ...
    while((pile.size()>0) || (a_visiter.size() > 0) ){ //tant qu'il reste des sommets dans la pile ou qu'il reste des sommets a visiter, on continue
		...
		// vr: des () en trop dans : if (is_in_lst((ar.get_sf(), a_visiter))){//on verifie que le sommet voisin n'a pas deja ete visite
                if (is_in_lst(ar.get_sf(), a_visiter)){//on verifie que le sommet voisin n'a pas deja ete visite
		  // vr: non: int j = ind_elem(a_visiter, ar.get_sf()); //on recupere l'indice du sommet voisin dans la liste des sommets a visiter
		  // vr: ind_elem prend la string en premier argument
		  int j = ind_elem(ar.get_sf(), a_visiter); //on recupere l'indice du sommet voisin dans la liste des sommets a visiter
...
// vr: ne copie pas tes arguments ! passe les par référence
void parcourt_iteratif(Graphe_mat graph){
  //vr: mat_to_adj est une méthode de Graphe_adj pas une fonction globale
  //    Graphe_adj graphadj = mat_to_adj(graph); //on convertit la matrice d'adjacence en graphe adj
  // vr: ne laisse pas cette fonction MAT_TO_ADJ elle est juste là pour que ton code compile
    Graphe_adj graphadj = MAT_TO_ADJ(graph); //on convertit la matrice d'adjacence en graphe adj
...
void parcourt_largeur(Graphe_adj graph,int niveau, std::string som){ 
    ...
    if(niveau==0){   //on parcourt tout le graphe, comme pour le parcourt en profondeur
		...
		// vr: tu veux parcourir les arêtes adjacentes à s
		// vr: non std::vector<Arete> aretes = graph.get_som();
		// vr: ATTENTION IL FAUT RENVOYER UN VECTOR ARETE DU SOMMET s
		// PAS UN VECTOR VIDE COMME JE LE FAIS POUR QUE TON CODE COMPILE
		std::vector<Arete> aretes; // vr: ICI RENVOYER LE BON TRUC
