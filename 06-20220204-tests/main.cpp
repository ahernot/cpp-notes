...
int main() {
    ...
    Arete a("Ville1", "Ville2", 10.);
    std::map <std::string, std::vector<Arete>> graph_map;

    Graphe_adj graph (graph_map);
    graph.print_graph();

    // Arete a("Paris","Lyon",1.1), b("Paris","Rennes",9.2), c("Paris","Tour",4.2);
    // Arete d("Paris","Lille",0.2), e("Paris","Marseille",6.2);
    // Arete f("Lyon","Lille",2.3), g("Lyon","Toulouse",1.3), h("Lyon","Mozere",2.9);
