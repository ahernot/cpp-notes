// g++ -std=c++11 05-20210324-threading/matrix_prod_2.cpp -o mp
...
// change to float etc later

template<typename T>
std::vector<T> thread_mult (const Matrix<T>& mat, const std::vector<T>& vec, const int& n_threads) {

    auto thread_worker = [] (const int start, const int stop, const int mat_width, const int mat_height, Matrix<T>& mat, std::vector<T>& vec, std::vector<T>& result) {

        // Initialise temporary result vector
        std::vector<T> temp_result (mat_width);

        // Loop through calculations
        for (int pos = start; pos < stop; pos++) {

            int y_pos = std::floor(start / mat_width);
            int x_pos = start - (y_pos * mat_height);

            temp_result[y_pos] += mat(y_pos, x_pos) * vec[x_pos];
        }
    ...
    int mat_size = mat_width * mat_height;

    // Get step per thread
    int thread_step = std::ceil(mat_size / n_threads);

    // Initialise result
    std::vector<T> result = new std::vector<T> (mat_width);

    // Create threads
    std::vector<std::thread> threads;
    for (int i=0; i<n_threads; i++) {

        int start = i * thread_step;
        int stop = std::min(mat_size, start + thread_step);
        
        // Run thread
        threads.push_back( std::thread( thread_worker, start, stop, mat_width, mat_height, std::ref(mat), std::ref(vec), std::ref(result) ) );
    }
